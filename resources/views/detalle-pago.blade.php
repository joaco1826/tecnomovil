            ...
            <div class="account">
                ...
                <div class="account__right">
                    ...
                    use Gloudemans\Shoppingcart\Facades\Cart;
                        @endphp
                        @foreach($payment->cart as $c)
                            @php
                                $pro = \App\Models\Product::find($c->id);
                            @endphp
                            <table class="table-buy" style="width: 100% !important;">
                                <tr>
                                    <td><img src="https://admin.tecnomovildelcaribe.com/{{ $c->image }}" alt="tecnomovil del caribe product"></td>
                                    <td>{{ $c->name }}<br>Cantidad: {{ $c->quantity }} <p>${{ number_format($c->price,0,',','.') }}</p></td>
                                </tr>
                            </table>
                        @endforeach
                        ...
                        <div class="c-summary clearfix width-100">
                            ...
                            <div class="info-address border-blue float-right margin-right-0">
                                <div class="subtotal align-middle row">
                                    ...
                                    <div class="columns medium-6">${{ number_format($payment->total - $payment->shipping,0,',','.') }}</div>
                                </div>
                                <div class="subtotal align-middle row">
                                    <div class="columns medium-6">Envío</div>
                                    <div class="columns medium-6">${{ number_format($payment->shipping,0,',','.') }}</div>
                                </div>
                                <div class="subtotal align-middle row" style="border: 0">
                                    <div class="columns medium-6">Total</div>
                                    <div class="columns medium-6">${{ number_format($payment->total,0,',','.') }}</div>
                                </div>
                            </div>
                        </div>
                    </div>
